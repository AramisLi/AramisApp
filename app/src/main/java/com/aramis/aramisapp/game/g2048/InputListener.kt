 ...
 *Description:
 */
class InputListener(private val mView: G2048View) : View.OnTouchListener {
    private var x = 0f
    private var y = 0f
    ...
    override fun onTouch(v: View, event: MotionEvent): Boolean {
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                previousY = y
                hasMoved = false
//                beganOnIcon =iconPressed(mView.sxnew)
            }
            MotionEvent.ACTION_MOVE -> {
                ...
                if (mView.game.isActive() && !beganOnIcon) {
                    ...
                    if (pathMoved() > SWIPE_MIN_DISTANCE * SWIPE_MIN_DISTANCE && !hasMoved) {
                        var moved = false
                        //Vertical 下移
                        if (((dy >= SWIPE_THRESHOLD_VELOCITY && abs(dy) >= abs(dx)) || y - startingY >= MOVE_THRESHOLD)
                            ...
                            logE("下移 下移")
                            //上移
                        } else if (((dy <= -SWIPE_THRESHOLD_VELOCITY && abs(dy) >= abs(dx)) || y - startingY <= -MOVE_THRESHOLD)
                        ...
                        //Horizontal 右移
                        if (((dx >= SWIPE_THRESHOLD_VELOCITY && abs(dx) >= abs(dy)) || x - startingX >= MOVE_THRESHOLD)
                            ...
                            logE("右移 右移")
                            //左移
                        } else if (((dx <= SWIPE_THRESHOLD_VELOCITY && abs(dx) >= abs(dy)) || x - startingX <= -MOVE_THRESHOLD)
